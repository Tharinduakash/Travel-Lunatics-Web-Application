...
export default function TravelBuddiesPage() {
  ...
  async function toggleApproval(id: number, value: boolean) {
    ...
    if (res.ok) { toast.success(value ? 'Approved' : 'Rejected'); fetchReviews() }
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Travel Buddy Reviews" description={`${total} submissions`} />
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {loading ? Array.from({ length: 5 }).map((_, i) => (
                <tr key={i}>{Array.from({ length: 7 }).map((_, j) => <td key={j} className="px-4 py-3"><div className="h-4 rounded bg-muted animate-pulse w-20" /></td>)}</tr>
              )) : reviews.length === 0 ? (
                ...
                <tr key={r.id} className="hover:bg-muted/20 transition">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      {r.image_url ? <img src={r.image_url} alt={r.name} className="h-8 w-8 rounded-full object-cover" /> :
                  ...
                  <td className="px-4 py-3 text-amber-500">{r.rating ? `${r.rating}/5` : '—'}</td>
                  <td className="px-4 py-3 max-w-xs"><p className="truncate text-sm text-muted-foreground">{r.review}</p></td>
                  <td className="px-4 py-3">
                    <StatusBadge status={r.approved ? 'approved' : 'pending'} />
                  </td>
                  <td className="px-4 py-3 text-muted-foreground whitespace-nowrap text-xs">{format(new Date(r.created_at), 'MMM d, yyyy')}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <button onClick={() => setSelected(r)} className="rounded p-1.5 hover:bg-accent transition" title="View"><Eye className="h-4 w-4" /></button>
                      {!r.approved ? (
                      ...
                      <button onClick={() => setDeleteId(r.id)} className="rounded p-1.5 text-destructive hover:bg-destructive/10 transition"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setSelected(null)}>
          <div className="w-full max-w-lg rounded-xl border bg-card p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-2 text-sm">
              {[['Email', selected.email], ['Country', selected.country || '—'], ['Travel Type', selected.travel_type || '—'],
                ['Places', Array.isArray(selected.places) ? selected.places.join(', ') : '—'],
                ['Rating', `${selected.rating}/5`], ['Status', selected.approved ? 'Approved' : 'Pending']].map(([l, v]) => (
                <div key={l} className="flex gap-3"><span className="w-28 shrink-0 font-medium text-muted-foreground">{l}</span><span>{v}</span></div>
              ))}
              ...
              <div className="flex gap-2 pt-2">
                {!selected.approved ? (
                  <button onClick={() => { toggleApproval(selected.id, true); setSelected(null) }}
                    className="flex-1 rounded-lg bg-green-600 py-2 text-sm font-medium text-white hover:bg-green-700 transition">Approve</button>
                ) : (
                  <button onClick={() => { toggleApproval(selected.id, false); setSelected(null) }}
                    className="flex-1 rounded-lg bg-orange-500 py-2 text-sm font-medium text-white hover:bg-orange-600 transition">Revoke Approval</button>
                )}
                <button onClick={() => { setDeleteId(selected.id); setSelected(null) }}
                  className="flex-1 rounded-lg bg-destructive py-2 text-sm font-medium text-destructive-foreground hover:bg-destructive/90 transition">Delete</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <ConfirmDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)} title="Delete Review" description="This will permanently delete this travel buddy review." onConfirm={deleteReview} loading={deleting} />
    </div>
  )
}
